...
class ShopifyInsightsFetcher {
  ...
  async extractInsights() {
    ...
    try {
      ...
      this.showError(
        error.message || "An error occurred while extracting insights"
      );
    }
  }
  ...
  displayResults(data) {
    ...
    const hasProducts =
      (data.hero_products && data.hero_products.length > 0) ||
                ...
                <div class="col-md-8">
                    ...
                    ${
                      brand.about_us_content
                        ...
                        <p class="mb-3">${this.truncateText(
                          brand.about_us_content,
                          300
                        )}</p>
                    `
    ...
    if (data.hero_products && data.hero_products.length > 0) {
      ...
      setTimeout(() => {
        ...
        const additionalProducts =
          document.getElementById(additionalProductsId);
        ...
        if (showMoreButton && additionalProducts) {
          ...
          showMoreButton.addEventListener("click", () => {
            if (isExpanded) {
              ...
              showMoreButton.innerHTML = `<i class="fas fa-chevron-down me-1"></i>Show ${
                data.hero_products.length - 6
              } More Hero Products`;
              isExpanded = false;
            } else {
  ...
  createProductCard(product) {
    ...
    if (product.price !== null && product.price !== undefined) {
      ...
      if (product.formatted_price) {
        ...
        this.currentCurrencyMode === "original" &&
                ...
                <div class="product-card">
                    <div class="d-flex">
                        ...
                        <div class="flex-grow-1">
                            <h6 class="mb-1">${this.truncateText(
                              product.title,
                              50
                            )}</h6>
  ...
  formatPolicyContent(content) {
    ...
    return content
      ...
      .replace(
        /(PRIVACY POLICY|TERMS OF SERVICE|RETURN POLICY|REFUND POLICY)/gi,
        "<h6>$1</h6>"
      );
  }
  ...
  populateSocialContact(data) {
    ...
    const foundSocial = socialPlatforms.filter(
    ...
    if (foundSocial.length > 0) {
      foundSocial.forEach((platform) => {
        html += `
                    <a href="#" class="social-handle" style="border-left: 4px solid ${
                      platform.color
                    }">
        ...
        html += `
                    <div class="col-md-6 mb-2">
                        <div class="link-item">
                            ...
                            <a href="${links[item.key]}" target="_blank">${
          item.label
        }</a>
                        </div>
                    </div>
      ...
      foundPolicies.forEach((item) => {
        html += `
                    <div class="mb-2">
                        ...
                        <a href="#" class="policy-link" data-policy-url="${
                          policies[item.key]
                        }" data-policy-title="${item.label}">${item.label}</a>
                    </div>
                `;
      });
    } else {
    ...
    if (faqs && faqs.length > 0) {
      ...
      faqs.slice(0, 5).forEach((faq, index) => {
        html += `
                    <div class="faq-item">
                        ...
                        <div class="faq-answer">${this.truncateText(
                          faq.answer,
                          150
                        )}</div>
                    </div>
      ...
      if (faqs.length > 5) {
        html += `<small class="text-muted">+${
          faqs.length - 5
        } more FAQs available in full dataset</small>`;
      }
    } else {
  ...
  populateAIValidation(data) {
    ...
    const aiValidationHtml = `
            ...
            ${
              ...
              validation.validation_notes.length > 0
                ? `
                <div class="mt-3">
  ...
  populateCompetitorAnalysis(data) {
    if (
      ...
      data.competitor_analysis.competitors_found === 0
    ) {
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <ul class="list-unstyled">
                                                    ${competitor.strengths
                                                      .map(
                                                        (strength) =>
                                                          `<li><i class="fas fa-plus-circle text-success me-1"></i>${strength}</li>`
                                                      )
                                                      .join("")}
                                                </ul>
                                            </div>
                                            ...
                                            <div>
                                                ...
                                                <ul class="list-unstyled">
                                                    ${competitor.weaknesses
                                                      .map(
                                                        (weakness) =>
                                                          `<li><i class="fas fa-exclamation-triangle text-warning me-1"></i>${weakness}</li>`
                                                      )
                                                      .join("")}
                                                </ul>
                                            </div>
  ...
  toggleCurrency() {
    ...
    this.currentCurrencyMode =
      this.currentCurrencyMode === "original" ? "usd" : "original";

    // Update toggle button text
    const toggleBtn = document.getElementById("currencyToggle");
    if (this.currentCurrencyMode === "original") {
      toggleBtn.innerHTML =
        '<i class="fas fa-dollar-sign me-1"></i>Switch to USD';
      toggleBtn.className = "btn btn-outline-secondary btn-sm";
    } else {
      toggleBtn.innerHTML =
        '<i class="fas fa-globe me-1"></i>Switch to Original';
  ...
  downloadJSON() {
    ...
    link.href = url;
    link.download = `shopify-insights-${
  ...
  setupWebsiteAutocomplete() {
    ...
    dropdown.className =
      "autocomplete-dropdown list-group position-absolute w-100";
    ...
    input.addEventListener("input", () => {
      ...
      const matches = this.websiteList
        .filter(
          (site) =>
            site.url.toLowerCase().includes(value) ||
            (site.name && site.name.toLowerCase().includes(value))
        )
        .slice(0, 8);
      if (matches.length === 0) {
        dropdown.style.display = "none";
